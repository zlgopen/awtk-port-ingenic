...
static void report_touch(run_info_t* info, int x, int y, int pressure)
{
    event_queue_req_t* req = &(info->req);

    /* printf("\033[33m x = %d | y = %d | pressure = %d |\033[0m\n", x, y, pressure); */
...
static ret_t tslib_dispatch_one_event(run_info_t* info) {
  ...
  // touch test
  static FILE *fd = NULL;
  static int tb1 = 0, tb2 = 0;
#define TEST_FILE "/tmp/touch.test"

  if (access(TEST_FILE, 0) == 0) {
      char buf[50];

      if (fd) {
	  int x, y, p, num;
	  unsigned int delay, t1, t2;
	  ...
	  buf[10] = ' ';
	  buf[17] = ' ';
	  /* printf("\033[33m buf = %s |\033[0m\n", buf); */
	  num = sscanf(buf+5, "%d %d %d %d %d", &t1, &t2, &x, &y, &p);

	  if (num != 5)
	      return RET_OK;

	  if (tb1 != 0) {
	      delay = (t1 - tb1) * 1000000 + (t2 - tb2);
	      if (delay < 20 * 1000000) {
		  /* printf("\033[33m delay = %d |\033[0m\n", delay); */
		  usleep(delay);
	      }
	  }
	  report_touch(info, x, y, p);
