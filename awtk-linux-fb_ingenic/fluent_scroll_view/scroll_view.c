...
static ret_t scroll_view_change_bitmap(widget_t* widget){
    scroll_view_t* scroll_view = SCROLL_VIEW(widget);
    return_value_if_fail(scroll_view != NULL && widget != NULL, RET_BAD_PARAMS);
    if(scroll_view->bitmap.data == NULL)
        return RET_OK;
    //init lcd
    canvas_t* c = NULL;
    window_manager_default_t* wm = WINDOW_MANAGER_DEFAULT(window_manager());
    c = native_window_get_canvas(wm->native_window);
    lcd_mem_t* mem = (lcd_mem_t*)(c->lcd);
    uint8_t* fbuff = mem->offline_fb;
    bitmap_t fb;
    bitmap_init(&fb, c->lcd->w, c->lcd->h, mem->format, fbuff);

    //draw bitmap
    rect_t r_save = rect_init(0, 0, c->lcd->w, c->lcd->h);
    int32_t xoffset = scroll_view->xoffset;
    int32_t yoffset = scroll_view->yoffset;
    color_t color_save = c->lcd->fill_color;
    color_t color_tran = color_init(0,0,0,0);
    rect_t r = rect_init(c->ox, c->oy, widget->w, widget->h);
    ENSURE(canvas_begin_frame(c, &r_save, LCD_DRAW_OFFLINE) == RET_OK);
    if(c->ox == 0 || c->oy ==0)
        canvas_translate(c, r.x, r.y);
    if(c->lcd->draw_mode != LCD_DRAW_OFFLINE)
        c->lcd->draw_mode = LCD_DRAW_OFFLINE;

    canvas_set_fill_color(c,color_tran);
    ...
    if(xoffset + r.w > scroll_view->bitmap.w)
        r.w = scroll_view->bitmap.w - xoffset;
    if(yoffset + r.h > scroll_view->bitmap.h)
        r.h = scroll_view->bitmap.h - yoffset;
    if(xoffset < 0){
        r.x = -xoffset;
        r.w = r.w + xoffset;
        xoffset = 0;
    ...
    image_copy(&scroll_view->bitmap, &fb, &r, xoffset, yoffset);

    ENSURE(canvas_end_frame(c) == RET_OK);
    //recovery lcd
    ENSURE(canvas_begin_frame(c, &r_save, LCD_DRAW_NORMAL) == RET_OK);
    ENSURE(widget_paint(window_manager(), c) == RET_OK);
    canvas_set_fill_color(c,color_save);
    r.x = 0;
    canvas_translate(c, r.x, r.y);
    return RET_OK;
}

static ret_t scroll_view_init_static_bitmap(widget_t* widget){
    scroll_view_t* scroll_view = SCROLL_VIEW(widget);
    return_value_if_fail(scroll_view != NULL && widget != NULL, RET_BAD_PARAMS);
    if(scroll_view->init_static){
        //init bitmap
        canvas_t* c = NULL;
        window_manager_default_t* wm = WINDOW_MANAGER_DEFAULT(window_manager());
        c = native_window_get_canvas(wm->native_window);
        lcd_mem_t* mem = (lcd_mem_t*)(c->lcd);
        bitmap_destroy(&scroll_view->bitmap);
        int virtual_w = tk_max(scroll_view->virtual_w, widget->w);
        int virtual_h = tk_max(scroll_view->virtual_h, widget->h);
        return_value_if_fail(bitmap_init(&scroll_view->bitmap, virtual_w, virtual_h, mem->format, NULL) == RET_OK,RET_OOM);
        uint8_t* fbuff = mem->offline_fb;
        bitmap_t fb;
        bitmap_init(&fb, c->lcd->w, c->lcd->h, mem->format, fbuff);
        ...
        if(c->lcd->draw_mode != LCD_DRAW_OFFLINE)
            c->lcd->draw_mode = LCD_DRAW_OFFLINE;
        while(yoffset < scroll_view->bitmap.h){
            while(xoffset < scroll_view->bitmap.w){
                canvas_set_fill_color(c,color_tran);
                canvas_fill_rect(c,0,0,r.w,r.h);
                canvas_translate(c, -xoffset, -yoffset);
                r = rect_intersect(&r, &r_save);
  ...
  switch (type) {
    ...
    case EVT_POINTER_MOVE: {
      ...
      if (scroll_view->dragged) {
        ...
        scroll_view_invalidate_self(widget);
        if(scroll_view->first_drag){
            scroll_view_change_bitmap(widget);
            scroll_view->first_drag = FALSE;
        }
      } else {
        int32_t delta = 0;
...
static ret_t scroll_view_on_paint_children(widget_t* widget, canvas_t* c) {
    ...
    scroll_view_t* scroll_view = SCROLL_VIEW(widget);
    scroll_view_init_static_bitmap(widget);

    if(!scroll_view->dragged && !scroll_view->wa){
        int32_t xoffset = -scroll_view->xoffset;
        int32_t yoffset = -scroll_view->yoffset;
        ...
        canvas_set_clip_rect(c, &r_save);
        canvas_untranslate(c, xoffset, yoffset);
    }else if(scroll_view->bitmap.data != NULL){
        ...
        if(xoffset + r.w > scroll_view->bitmap.w){
            w = scroll_view->bitmap.w - xoffset;
        }
        if(yoffset < 0){
            sy = 0;
            y = r.y-yoffset;
        ...
        if(yoffset + r.y > scroll_view->bitmap.h){
            h = scroll_view->bitmap.h - yoffset;
        }
        rect_t src = rect_init(sx,sy,w,h);
        rect_t dst = rect_init(x,y,w,h);
        lcd_draw_image(c->lcd, &scroll_view->bitmap, &src, &dst);
    }
  return RET_OK;
}
...
static ret_t scroll_view_on_destroy(widget_t* widget) {
  scroll_view_t* scroll_view = SCROLL_VIEW(widget);
  return_value_if_fail(scroll_view != NULL , RET_BAD_PARAMS);
  if(scroll_view->bitmap.data != NULL){
        bitmap_destroy(&scroll_view->bitmap);
...
TK_DECL_VTABLE(scroll_view) = {.size = sizeof(scroll_view_t),
                               ...
                               .on_destroy = scroll_view_on_destroy};
